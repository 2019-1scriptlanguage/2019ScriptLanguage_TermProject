...
def map_window(address):    # example ) address = '정왕동 2314-21'
    window = Tk()
    window.wm_title("지도")
    map_url = make_googlemap_url(Parsing_KAKAOMAP_Address(address)[0])
    with urlopen(map_url) as u:
        raw_data = u.read()
    im = Image.open(BytesIO(raw_data))
    map_image = ImageTk.PhotoImage(im, master=window)
    Label(window, image=map_image, height=500, width=500, background='white').pack()
